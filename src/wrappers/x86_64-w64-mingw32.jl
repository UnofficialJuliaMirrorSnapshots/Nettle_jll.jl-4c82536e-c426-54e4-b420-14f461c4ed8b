...
const libhogweed_splitpath = ["bin", "libhogweed-4.dll"]
...
const libhogweed = "libhogweed-4.dll"


# Relative path to `libnettle`
const libnettle_splitpath = ["bin", "libnettle-6.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libnettle = "libnettle-6.dll"


# Relative path to `nettle_hash`
const nettle_hash_splitpath = ["bin", "nettle-hash.exe"]
